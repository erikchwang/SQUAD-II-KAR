...
with tf.Session(
    ...
    while True:
        early_stopping_trigger_count = sum(
            model_progress[index]["HasAns_f1"] <= max(item["HasAns_f1"] for item in model_progress[:index]) or
            model_progress[index]["NoAns_f1"] <= max(item["NoAns_f1"] for item in model_progress[:index])
        ...
        for batch in [train_queue[index:index + batch_size] for index in range(0, len(train_queue), batch_size)]:
            ...
            for index in range(batch_size):
                ...
                feed_dict[ANALOG_SPAN_BATCH[index]] = feed_batch[index]["analog_span"]
                feed_dict[ANSWER_CLASS_BATCH[index]] = feed_batch[index]["answer_class"]
        ...
        for batch in [develop_queue[index:index + batch_size] for index in range(0, len(develop_queue), batch_size)]:
            ...
            for record, predict in zip(feed_batch, model_predicts):
                develop_solution[record["question_id"]] = "" if predict[0] == 0 else spacy_nlp(
                    record["passage_string"]
                )[predict[1]:predict[2] + 1].text
