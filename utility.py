...
def feed_forward(
    ...
    with tf.variable_scope("OUTPUT"):
            ...
            input=tf.layers.dense(
                inputs=tf.concat(
                    values=[
                        ...
                        QUESTION_SUMMARY_CODES[0],
                        QUESTION_SUMMARY_CODES[1]
                    ],
                    axis=1
                ),
                units=2,
        ...
        return ANSWER_SPAN_DISTRIBUTION, ANALOG_SPAN_DISTRIBUTION, ANSWER_CLASS_DISTRIBUTION
...
def build_update(
        ...
        ANSWER_SPAN_BATCH, ANALOG_SPAN_BATCH, ANSWER_CLASS_BATCH, LEARNING_RATE, EMA_MANAGER
):
    GRADIENTS_BATCH = []
    ...
    for index in range(batch_size):
        with tf.device("/device:GPU:{}".format(index % gpu_count)):
            with tf.variable_scope(name_or_scope=tf.get_variable_scope(), reuse=True if index > 0 else None):
                ANSWER_SPAN_DISTRIBUTION, ANALOG_SPAN_DISTRIBUTION, ANSWER_CLASS_DISTRIBUTION = feed_forward(
                    ELMO_MODULE, WORD_EMBEDDING,
...
def build_predicts(
    ...
    def get_greedy_sample(SUBJECT_DISTRIBUTION, OBJECT_DISTRIBUTION):
        return tf.unravel_index(
                    ...
                    tf.reshape(
                        tensor=tf.linalg.band_part(
                            ...
                            num_upper=tf.math.subtract(
                                x=tf.math.minimum(x=tf.shape(OBJECT_DISTRIBUTION)[1], y=answer_span_length_limit),
                                y=1
                            )
                        ),
                        shape=[-1]
            ...
            dims=[tf.shape(OBJECT_DISTRIBUTION)[1], tf.shape(OBJECT_DISTRIBUTION)[1]]
        )
